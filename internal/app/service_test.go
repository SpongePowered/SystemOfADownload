...
func TestService_GetGroup(t *testing.T) {
	...
	tests := []struct {
		name      string
		groupID   string
		mockSetup func(m *dbmocks.MockQuerier)
		want      *domain.Group
		wantErr   error
	}{
		{
	...
	for _, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			mockQ := dbmocks.NewMockQuerier(t)
			if tt.mockSetup != nil {
				tt.mockSetup(mockQ)
			}

			svc := app.NewService(mockQ)
...
func TestService_RegisterGroup(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			mockSetup: func(m *dbmocks.MockQuerier) {
				m.EXPECT().GroupExistsByMavenID(mock.Anything, "g1").Return(false, nil)
				m.EXPECT().CreateGroup(mock.Anything, db.CreateGroupParams{
					MavenID: "g1",
					Name:    "Group",
		...
		},
		{
			...
			mockSetup: func(m *dbmocks.MockQuerier) {
				m.EXPECT().GroupExistsByMavenID(mock.Anything, "existing.group").Return(true, nil)
			},
		...
		},
		{
			...
			mockSetup: func(m *dbmocks.MockQuerier) {
				m.EXPECT().GroupExistsByMavenID(mock.Anything, "g3").Return(false, errors.New("db check failed"))
			},
		...
		},
		{
			...
			mockSetup: func(m *dbmocks.MockQuerier) {
				m.EXPECT().GroupExistsByMavenID(mock.Anything, "g2").Return(false, nil)
				m.EXPECT().CreateGroup(mock.Anything, db.CreateGroupParams{
					MavenID: "g2",
					Name:    "Group 2",
					Website: nil,
